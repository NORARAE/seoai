...
<meta name="description" content="See how SEO AI Co™ structures your domain for AI citation — from mapping your market to deploying structured pages across every service and city you serve. Three steps to getting cited everywhere.">
...
<meta property="og:description" content="See how SEO AI Co™ structures your domain for AI citation — three steps to getting cited across Google, ChatGPT, and AI search.">
...
.tier-grid-4::before{
  content:'';
  position:absolute;top:-80px;left:50%;width:25%;height:calc(100% + 160px);
  background:radial-gradient(ellipse at 50% 38%,rgba(200,168,75,.06) 0%,transparent 68%);
  pointer-events:none;z-index:0;
}
.tier-grid-4 .tier:nth-child(1){transition-delay:.05s}
...
.tier.scan-tier .tier-flag{color:rgba(200,168,75,.35);font-size:.62rem}
.tier.scan-tier .tier-name{font-size:1.35rem;color:rgba(168,168,160,.78)}
.tier.scan-tier .tier-price{font-size:2.2rem;color:rgba(200,168,75,.3)}
.tier.scan-tier .tier-price sup{color:rgba(200,168,75,.3)}
.tier.scan-tier .tier-cta{
  color:rgba(200,168,75,.4);border:1px solid rgba(200,168,75,.14);font-size:.7rem;
}
.tier.scan-tier .tier-cta:hover{
  background:rgba(200,168,75,.06);border-color:rgba(200,168,75,.28);color:var(--gold-dim);
}
/* ── Report tier ── */
.tier.report-tier{
  background:var(--deep);
...
.tier.report-tier .tier-flag{color:rgba(200,168,75,.45);font-size:.65rem}
.tier.report-tier .tier-name{font-size:1.5rem;color:rgba(168,168,160,.86)}
.tier.report-tier .tier-price{font-size:2.6rem;color:rgba(200,168,75,.45)}
.tier.report-tier .tier-price sup{color:rgba(200,168,75,.45)}
.tier.report-tier .tier-cta{
  color:rgba(200,168,75,.5);border:1px solid rgba(200,168,75,.2);font-size:.72rem;
}
.tier.report-tier .tier-cta:hover{
  background:rgba(200,168,75,.06);border-color:rgba(200,168,75,.35);color:var(--gold);
}
.tier:hover{
...
.nav-menu{
  ...
  z-index:9200;
  background:linear-gradient(160deg,#0f0e09 0%,#0a0908 60%,#0d0b06 100%);
...
.nm-link{
  ...
  color:rgba(210,210,202,.78);text-decoration:none;
...
.nm-link.nm-muted::after{color:rgba(200,168,75,.18);font-size:.9rem}
.nm-link.nm-muted:hover{color:rgba(168,168,160,.72);background:transparent}
.nm-link.nm-muted:hover::after{color:rgba(200,168,75,.35)}
...
.nm-signin{
  ...
  color:rgba(168,168,160,.4);text-decoration:none;
  transition:color .18s ease;
  min-height:44px;
}
.nm-signin::after{content:'›';color:rgba(200,168,75,.18);font-size:.9rem;transition:color .18s ease,transform .18s ease}
.nm-signin:hover{color:rgba(168,168,160,.75)}
.nm-signin:hover::after{color:rgba(200,168,75,.45);transform:translateX(2px)}
...
#heroSeq{
  ...
  /* Visible by default so headline shows even if JS fails */
  opacity:1;transform:translateY(0);
}
/* JS adds .hs-anim to opt-in to transitions */
#heroSeq.hs-anim{
...
<script type="application/ld+json">
{
  ...
  "@graph": [
    {
      "@type": "WebPage",
      "@id": "https://seoaico.com/how-it-works#webpage",
      "url": "https://seoaico.com/how-it-works",
      "name": "How the AI Citation Engine™ Works",
      "description": "See how SEO AI Co™ structures your domain for AI citation — from mapping your market to deploying structured pages across every service and city you serve.",
      "breadcrumb": {
        "@type": "BreadcrumbList",
        "itemListElement": [
          {"@type": "ListItem", "position": 1, "name": "Home", "item": "https://seoaico.com"},
          {"@type": "ListItem", "position": 2, "name": "How It Works", "item": "https://seoaico.com/how-it-works"}
        ]
      }
    },
    {
...
<section id="hero">
  ...
  <div class="hero-stage">
    <h1 id="heroSeq" aria-label="How the AI Citation Engine works — from scan to market dominance">How the AI Citation<br>Engine™ Works</h1>
  </div>
  <p class="hero-gold-accent">From a $2 scan to full market dominance — one expanding system.</p>
  <div class="hero-actions" style="opacity:0;animation:up .85s .52s forwards">
    ...
    <a href="#how" class="btn-ghost">See the process</a>
  </div>

</section>

<div class="hero-transition">
  <a href="#how" class="hero-scroll-label" style="text-decoration:none">The system, explained</a>
...
<section id="alloc" class="alloc-section r">
  <div class="alloc-layout">
    ...
    <div class="alloc-copy">
      ...
      <p class="alloc-access-note">Instant score · See what AI systems can read</p>
    </div>
...
{{-- ── Deployment positioning block ── --}}
<section class="r" style="max-width:780px;margin:0 auto;padding:56px 40px;text-align:center">
  <p style="font-size:.62rem;letter-spacing:.28em;text-transform:uppercase;color:rgba(200,168,75,.55);margin-bottom:14px">Deployment</p>
  <h2 style="font-family:'Cormorant Garamond',serif;font-size:clamp(1.5rem,3vw,2rem);font-weight:300;color:var(--ivory);line-height:1.2;margin-bottom:16px">Built to operate within <em style="font-style:italic;color:var(--gold-lt)">your existing site.</em></h2>
  <p style="font-size:.92rem;color:var(--muted);line-height:1.8;max-width:560px;margin:0 auto 24px">The system deploys as a structured expansion layer &#8212; extending your environment with citation-ready architecture. No migration. No rebuild. No second platform.</p>
  <div style="display:flex;justify-content:center;gap:32px;flex-wrap:wrap;margin-bottom:20px">
    <span style="font-size:.68rem;letter-spacing:.16em;text-transform:uppercase;color:rgba(200,168,75,.65)">No migration</span>
    <span style="font-size:.68rem;letter-spacing:.16em;text-transform:uppercase;color:rgba(200,168,75,.65)">No rebuild</span>
    <span style="font-size:.68rem;letter-spacing:.16em;text-transform:uppercase;color:rgba(200,168,75,.65)">Extends your environment</span>
  </div>
  <p style="font-size:.78rem;color:rgba(168,168,160,.5);line-height:1.6;max-width:520px;margin:0 auto">For many clients, this integrates directly with WordPress. For others, it operates as a custom PHP-driven expansion layer. The system adapts to your infrastructure &#8212; not the other way around.</p>
</section>
  ...
  <div class="audience-section">
    ...
    <div class="audience-grid">
      ...
      <div class="aud-card r">
        ...
        <a href="{{ route('quick-scan.show') }}" class="aud-cta">See Where You Stand — $2 &rarr;</a>
      </div>
...
<script>
  ...
  (function(){
    ...
    // Opt-in to transitions now that JS is confirmed running
    el.classList.add('hs-anim');

    // Reduced-motion: keep initial headline, no cycling
    if(window.matchMedia('(prefers-reduced-motion:reduce)').matches) return;

    // Start cycling after first hold period
    setTimeout(function(){
      (function cycle(){
        // Fade out
