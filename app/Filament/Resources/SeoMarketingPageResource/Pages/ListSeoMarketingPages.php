...
class ListSeoMarketingPages extends ListRecords
{
    ...
    public function getTabs(): array
    {
        return [
            ...
            'core' => Tab::make('Core')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('cluster', 'core'))
                ->badge(fn () => SeoMarketingPage::where('cluster', 'core')->count())
                ->badgeColor('primary'),

            'agency' => Tab::make('Agency')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('cluster', 'agency'))
                ->badge(fn () => SeoMarketingPage::where('cluster', 'agency')->count())
                ->badgeColor('info'),

            'local' => Tab::make('Local')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('cluster', 'local'))
                ->badge(fn () => SeoMarketingPage::where('cluster', 'local')->count())
                ->badgeColor('success'),

            'strategy' => Tab::make('Strategy')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('cluster', 'strategy'))
                ->badge(fn () => SeoMarketingPage::where('cluster', 'strategy')->count())
                ->badgeColor('warning'),

            'industry' => Tab::make('Industry')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('cluster', 'industry'))
                ->badge(fn () => SeoMarketingPage::where('cluster', 'industry')->count())
                ->badgeColor('danger'),

            'money_pages' => Tab::make('Money Pages')
                ->modifyQueryUsing(fn (Builder $query) => $query->whereNotNull('money_page_rank')->orderBy('money_page_rank'))
